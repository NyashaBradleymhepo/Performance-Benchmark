// Load data from forward_player_data.csv (assuming it contains forward player data)
d3.csv("data.csv").then(function(data) {
    // Filter data for forwards
    const forwardsData = data.filter(d => d.Position === "Forwards");

    // Remove duplicate forward players and calculate the average Power Ranking for each unique player
    const playerAverages = d3.group(forwardsData, d => d.Player);
    ...
    // Select the table body for the forward player table
    const tbody = d3.select("#forward-player-table tbody"); // Use the id of the forward player table

    // Bind data to forward player table rows
    const rows = tbody.selectAll("tr")
        .data(top10UniqueData)
        .enter()
        .append("tr");

    // Add positions to the first column of the forward player table
    rows.append("td")
        .text(d => d.Position) // Use the "Position" field

    // Add forward player names (from "Player" column) to the second column of the forward player table
    rows.append("td")
        .text(d => d.Player) // Use the "Player" column
        .classed("player-name-cell", true); // Add a class for styling

    // Add average power rankings (from "AverageRanking" column) to the third column of the forward player table
    rows.append("td")
        .text(d => d.AverageRanking); // Use the "AverageRanking" column

    // Select all forward player name cells in the forward player table
    const playerNameCells = d3.selectAll("#forward-player-table tbody tr .player-name-cell"); // Use the id of the forward player table
